#!/bin/bash
# launch_kfragd_100ms_no_force_reclaim.sh — kfragd no-force-reclaim + 100ms interval + 16KB THP 实验启动器
# 用法: bash scripts/launch_kfragd_100ms_no_force_reclaim.sh
...
OUT_DIR="${OUT_DIR:-$HOME/learn_os/.worklog/thp_16k_kfragd_100ms_no_force_reclaim_$(date +%Y%m%d_%H%M%S)}"
...
set_dev "kfragd_force_reclaim" /proc/sys/vm/kfragd_force_reclaim 0
...
REQS=(
  ...
  "kfragd_force_reclaim|/proc/sys/vm/kfragd_force_reclaim|0"
...
say "out_dir=$OUT_DIR"
cd "$SKILL_DIR" || exit 1
say "tasktime procs: kfragd0, kswapd0 (按名解析, 缺失自动跳过)"
exec python3 "$RUNNER" \
  --serial "$SERIAL" \
  --from-manifest "$MANIFEST" \
  --tasktime-procs kfragd0,kswapd0 \
  --out "$OUT_DIR"
