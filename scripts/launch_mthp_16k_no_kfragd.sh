#!/bin/bash
# launch_mthp_16k_no_kfragd.sh — 16kB mTHP 全开 + 无 kfragd 实验启动器
# 用法: bash scripts/launch_mthp_16k_no_kfragd.sh
#   默认: 重启进 bootloader -> 刷 vendor_boot_default.img -> 重启 -> 自检 -> 实验
# 实验定位: anon 16kB THP + 文件页 large folio(order2) 全开, kfragd 不干预
# 自检要求(必须全部满足, 否则拒绝启动):
#   - fs_disable_large_folio = 0 (boot 参数不能开, 否则文件页被禁为 order0)
#   - kfragd_enabled = 0 (kfragd 不能开)
#   - THP 16kB enabled = always (mthp 开着)
#   - f2fs/ext4 min/max_folio_order_cap = 2 (文件页 large folio 全开 order2)
set -u -o pipefail
...
OUT_DIR="${OUT_DIR:-$HOME/learn_os/.worklog/mthp_16k_no_kfragd_$(date +%Y%m%d_%H%M%S)}"
...
set_dev "f2fs min_folio_order_cap" /sys/fs/f2fs/dm-49/min_folio_order_cap 2
set_dev "f2fs max_folio_order_cap" /sys/fs/f2fs/dm-49/max_folio_order_cap 2
set_dev "ext4 min_folio_order_cap" /sys/fs/ext4/min_folio_order_cap 2
set_dev "ext4 max_folio_order_cap" /sys/fs/ext4/max_folio_order_cap 2
...
ok="OK"; [ "$v" != "0" ] && { ok="FAIL(boot 参数 fs_disable_large_folio 必须=0)"; FAIL=1; }
...
echo "自检全部通过 (16kB mTHP 全开, 无 kfragd)"
